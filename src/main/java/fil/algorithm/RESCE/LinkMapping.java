...
public class LinkMapping {
	...
	public boolean linkMapExternal(Topology topo, LinkedList<SFC> listSFC) {
...
//							continue;
	...
	public boolean linkMapInternal(Topology topo, LinkedList<SFC> listSFC) {
...
//			vLink.getLinkSubstrate().addAll(linkEdge); // Kien add 1912
	...
	private boolean linkMappingCoreSeparate( VirtualLink vLink, SubstrateSwitch edgeSwitch1,
		...
		SubstrateSwitch aggA = null, aggB = null;	
		...
		boolean checkAgg1 = false; boolean checkAgg2 = false;
		...
		for(SubstrateSwitch agg1 : listAggSort1) {
			...
			for (SubstrateLink link : listPhyLinks) {
				...
				if(link.equals(listPhyLinks.getLast()) && count != 2) {
					checkAgg1 = false;
					break;
				}
				...
				for(SubstrateSwitch agg2 : listAggSort2) {
					...
					if(checkAgg2) {
						...
						if(!listCoreSort1.equals(listCoreSort2)) {
							if(agg2.equals(listAggSort2.getLast())) {
								System.out.println("Int Link failed at Core4.");
								return false;
							}
							else
								continue;
						}else { // condition core switch comparison
							for(SubstrateSwitch core : listCoreSort1) {
								count = 0;
								for (SubstrateLink link : listPhyLinks) {
									if(link.getStart() == agg1 && link.getEnd() == core) {
										if(link.getRemainBW() < bwDemand) {
											break;
										}else {
											count ++;
									...
									else if(link.getStart() == core && link.getEnd() == agg1) {
										if(link.getRemainBW() < bwDemand) {
											break;
										}else {
											count ++;
									...
									else if(link.getStart() == agg2 && link.getEnd() == core) {
										if(link.getRemainBW() < bwDemand) {
											break;
										}else {
											count ++;
									...
									else if(link.getStart() == core && link.getEnd() == agg2) {
										if(link.getRemainBW() < bwDemand) {
											break;
										}else {
											count ++;
									...
									if(count == 4) {
										...
										return true;
									} // condition count core equals 4
								}
							}
						}
					}else { // condition checkAgg2
						if(agg2.equals(listAggSort2.getLast())) {
							System.out.println("Int Link failed at Core3.");
							return false;
						}
						else
							continue;
			...
			}else { // condition checkAgg1
				if(agg1.equals(listAggSort1.getLast())) {
					System.out.println("Int Link failed at Core2.");
					return false;
				}
				else
					continue;
	...
	public LinkedList<SubstrateLink> MapLink(LinkedList<SubstrateSwitch> path, LinkedList<SubstrateLink> listPhyLinksTemp, double bandwidth)
	{
		for (int i = 0; i < path.size() - 1; i++) {
		
			SubstrateSwitch switch1 = path.get(i);
			SubstrateSwitch switch2 = path.get(i + 1);
			for (int j = 0; j < listPhyLinksTemp.size(); j++) {
				SubstrateLink link = listPhyLinksTemp.get(j);
				// update bandwidth, two-direction
				if (link.getStart().equals(switch1) && link.getEnd().equals(switch2)) {
					link.setUsedBW(link.getRemainBW() - bandwidth);
					listPhyLinksTemp.set(j, link);
					// break;
				}
				//Vm1-> Vm2 == Vm2-Vm1
				if (link.getStart().equals(switch2) && link.getEnd().equals(switch1)) {
					link.setUsedBW(link.getRemainBW() - bandwidth);
					listPhyLinksTemp.set(j, link);
					// break;
				}
	...
	public double getBanwidthOfPath(LinkedList<SubstrateSwitch> path, LinkedList<SubstrateLink> listPhyLinks) {
		double bandwidth = Integer.MAX_VALUE;
		for (int i = 0; i < path.size() - 1; i++) {
			SubstrateSwitch switch1 = path.get(i);
			SubstrateSwitch switch2 = path.get(i + 1);
			for (int j = 0; j < listPhyLinks.size(); j++) {
				SubstrateLink link = listPhyLinks.get(j);
				if (link.getStart().equals(switch1) && link.getEnd().equals(switch2)) {
					if (link.getRemainBW() < bandwidth)
						bandwidth = link.getRemainBW();
					break;
				}
			}
		}
